...
int main( int argc, char *argv[] ){
  ...
  printf("\nProgram obliczania caĹki z funkcji (sinus) metodÄ trapezĂłw.\n");

  a = 0.0;
  //printf("\nPodaj lewy kraniec przedziaĹu caĹkowania: "); scanf("%lf", &a);

  b = M_PI;
  //printf("\nPodaj prawy kraniec przedziaĹu caĹkowania: "); scanf("%lf", &b);

  printf("\nPodaj wysokoĹÄ pojedynczego trapezu:  "); scanf("%lf", &dx);


  //printf("\nPodaj liczbÄ wÄtkĂłw:  "); scanf("%d", &l_w_global);

  printf("\nPoczatek obliczeĹ sekwencyjnych\n");
  t1 = czas_zegara();
  ...
  printf("\nPoczatek obliczeĹ rĂłwnolegĹych (zrĂłwnoleglenie pÄtli)\n");
  t1 = czas_zegara();

  calka = calka_zrownoleglenie_petli(a, b, dx);

  t1 = czas_zegara() - t1;
  printf("\nKoniec obliczen  rĂłwnolegĹych (zrĂłwnoleglenie pÄtli) \n");
  printf("\tCzas wykonania %lf. \tObliczona caĹka = %.15lf\n", t1, calka);

  printf("\nPoczatek obliczeĹ rĂłwnolegĹych (dekompozycja obszaru)\n");
  t1 = czas_zegara();

  calka = calka_dekompozycja_obszaru(a, b, dx);

  t1 = czas_zegara() - t1;
  printf("\nKoniec obliczen  rĂłwnolegĹych (dekompozycja obszaru) \n");
  printf("\tCzas wykonania %lf. \tObliczona caĹka = %.15lf\n", t1, calka);
...
double calka_sekw(double a, double b, double dx){
  ...
  printf("Obliczona liczba trapezĂłw: N = %d\n", N);
...
double calka_zrownoleglenie_petli(double a, double b, double dx){
  ...
  int l_w = l_w_global;
  //printf("\nPodaj liczbÄ wÄtkĂłw:  "); scanf("%d", &l_w);


  // tworzenie struktur danych do obsĹugi wielowÄtkowoĹci


  // tworzenie wÄtkĂłw


  // oczekiwanie na zakoĹczenie pracy wÄtkĂłw
...
void* calka_fragment_petli_w(void* arg_wsk){
  ...
  // a = a_global; // itd. itp. - wartoĹci globalne nadaje calka_zrownoleglenie_petli
...
void* calka_podobszar_w(void* arg_wsk){
  ...
  // rozpakowanie danych przesĹanych do wÄtku
